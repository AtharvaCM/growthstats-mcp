...
function parseSemrel(out) {
    ...
    return {
        ...
        releaseType: (typeMatch?.[1] ?? (versionMatch ? "patch" : "none")),
        notes
    };
}
function inferBumpFromCommits(commits) {
    ...
    for (const m of commits) {
        if (/BREAKING CHANGE|!:/i.test(m))
            major = true;
        else if (/^feat(\(|:)/i.test(m))
            minor = true;
        else if (/^(fix|perf|refactor|revert|chore|build|ci|docs|style|test)(\(|:)/i.test(m))
            patch = true;
    }
    if (major)
        return "major";
...
server.addTool({
    ...
    run: async ({ prTitle, commits }) => {
        ...
        const rank = (b) => ({ none: 0, patch: 1, minor: 2, major: 3 }[b] ?? 0);
        const violations = [];
        if (!declared)
            violations.push("PR title missing [Release][MAJOR|MINOR|PATCH] tag.");
...
server.addTool({
    ...
    run: async ({ repoPath, since, until }) => {
        const cwd = repoPath || env("REPO_PATH");
        ...
        for (const line of stdout.split("\n")) {
            ...
            if (/BREAKING CHANGE|!:/i.test(body) || /!:/i.test(subject))
                buckets.breaking.items.push(item);
            else if (/^feat(\(|:)/i.test(subject))
                buckets.feat.items.push(item);
            else if (/^fix(\(|:)/i.test(subject))
                buckets.fix.items.push(item);
            else if (/^perf(\(|:)/i.test(subject))
                buckets.perf.items.push(item);
            else
